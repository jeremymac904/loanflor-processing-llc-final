...
function downloadInstallScript(ref, destPath) {
  ...
  // Flo downstream (ADR-008): the runtime installer must come from a Flo-owned
  // source. With no channel configured, refuse instead of fetching the
  // upstream Hermes installer; the caller falls back to a local/installed
  // script or surfaces a clear bootstrap error.
  const floSource = floBootstrapSource()

  if (!floSource) {
    return Promise.reject(
      new Error(`Flo runtime bootstrap source is not configured; refusing to download the upstream Hermes ${scriptName}.`)
    )
  }
