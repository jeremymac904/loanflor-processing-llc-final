...
// Flo downstream: the visible product name comes from flo/brand.config.json. The
// env override is kept for upstream's dev-sandbox tooling.
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || FLO_BRAND.productName
...
if (IS_WINDOWS) {
  // Flo downstream: must equal package.json build.appId (enforced by
  // electron/flo-brand.test.ts against flo/brand.config.json).
  app.setAppUserModelId(FLO_BRAND.appId)
}
...
async function checkUpdates() {
  ...
  // Flo downstream (ADR-008): never check against the upstream Hermes
  // repository. Gate before any git/network traffic. See flo/release-channel.ts.
  const floGate = floUpdateGate(directoryExists(gitDir) ? await getOriginUrl(updateRoot).catch(() => '') : '')

  if (!floGate.allowed) {
    return {
      supported: false,
      reason: floGate.reason,
      message: floGate.message,
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  // Flo downstream (ADR-008): refuse to apply an update unless a Flo-owned
  // release channel is configured and the checkout tracks it. This also
  // covers the `connections:update-all` path, which calls applyUpdates
  // directly rather than through the IPC handler.
  {
    const floRoot = resolveUpdateRoot()
    const floOrigin = directoryExists(path.join(floRoot, '.git')) ? await getOriginUrl(floRoot).catch(() => '') : ''
    const floGate = floUpdateGate(floOrigin)

    if (!floGate.allowed) {
      emitUpdateProgress({ stage: 'error', message: floGate.message })
      throw new Error(floGate.message)
    }
  }
...
// ---------------------------------------------------------------------------
// Flo downstream: the OS-registered scheme is Flo's (flo/brand.config.json). The
// constant keeps its upstream name so the surrounding code stays mergeable.
const HERMES_PROTOCOL = DEV_SERVER ? `${FLO_BRAND.protocolScheme}-dev` : FLO_BRAND.protocolScheme
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER
  ? [`${FLO_BRAND.protocolScheme}-dev`, FLO_BRAND.protocolScheme]
  : [FLO_BRAND.protocolScheme]
