...
// Flo downstream: user-visible exe identity comes from flo/brand.config.json so
// the Explorer/Task Manager name, company and copyright match the product.
function readBrand(desktopRoot) {
  return JSON.parse(readFileSync(join(desktopRoot, 'flo', 'brand.config.json'), 'utf8'))
}
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  const brand = readBrand(desktopRoot)
