...
import { DEEP_LINK_SCHEMES } from '../../flo/brand'

export type HermesOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

// Flo downstream: the OS scheme is Flo's; the upstream scheme remains an
// internal alias so plugin/notification payloads keep resolving.
const HERMES_PROTOCOL = 'hermes:'
const ACCEPTED_SCHEME_PREFIXES = DEEP_LINK_SCHEMES.map(scheme => `${scheme}://`)
...
export function normalizeHermesOpenString(raw: string): string | null {
  ...
  if (ACCEPTED_SCHEME_PREFIXES.some(prefix => trimmed.startsWith(prefix)) || trimmed.startsWith(`${HERMES_PROTOCOL}//`)) {
    try {
