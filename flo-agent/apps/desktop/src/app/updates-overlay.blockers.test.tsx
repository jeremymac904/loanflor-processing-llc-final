...
// Flo downstream: expected literals pass through the same brand transform as the UI.
...
describe('BlockerView', () => {
  ...
  it('uses the blocker view for a foreign process instead of the generic update error', async () => {
    ...
    expect(screen.getByText(rebrandText('Close other processes to update Hermes'))).toBeTruthy()
  ...
  it('explains safe local previews and offers one-click close-and-update', async () => {
    ...
    expect(screen.getByText(rebrandText('Close local previews to update Hermes?'))).toBeTruthy()
    expect(screen.getByText('Example Preview')).toBeTruthy()
