...
describe('desktop i18n runtime translator', () => {
  ...
  it('translates string paths for the active runtime locale', () => {
    ...
    // Flo downstream: the product name is rebranded at the i18n boundary.
    expect(translateNow('boot.ready')).toBe('Flo 桌面版已就绪')
  ...
  it('falls back to English when the active locale cannot resolve a key', () => {
    ...
    try {
      ...
      // Flo downstream: "Hermes Desktop" → the Flo product name.
      expect(translateNow('boot.ready')).toBe('Flo is ready')
    } finally {
