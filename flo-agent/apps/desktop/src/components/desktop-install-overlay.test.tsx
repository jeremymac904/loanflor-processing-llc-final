...
// Flo downstream: screen text is rebranded at the i18n boundary; expected
// literals stay upstream-identical and pass through the same transform.
import { rebrandText } from '../../flo/brand'
...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText(rebrandText('Set up Hermes Desktop'))).toBeTruthy()
    expect(screen.getByText(rebrandText('Connect to existing Hermes'))).toBeTruthy()
    expect(screen.getByText(rebrandText('Install Hermes locally'))).toBeTruthy()
  ...
  it('continues local bootstrap only when Install Hermes locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText(rebrandText('Install Hermes locally')))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText(rebrandText('Set up Hermes Desktop'))).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText(rebrandText('Set up Hermes Desktop'))).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText(rebrandText('Install Hermes locally'))).closest(
      'button'
    ) as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText(rebrandText('Local installation could not start. Restart Hermes Desktop and try again.'))
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent(rebrandText('Install Hermes locally'))).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(
      screen.queryByText(rebrandText('Local installation could not start. Restart Hermes Desktop and try again.'))
    ).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click(
      (await screen.findByText(rebrandText('Install Hermes locally'))).closest('button') as HTMLButtonElement
    )
    expect(
      await screen.findByText(rebrandText('Local installation could not start. Restart Hermes Desktop and try again.'))
    ).toBeTruthy()
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText(rebrandText('Connect to existing Hermes')))
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText(rebrandText('Hermes needs a one-time install'))).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText(rebrandText('Hermes needs a one-time install'))).toBeNull()
  })
})
