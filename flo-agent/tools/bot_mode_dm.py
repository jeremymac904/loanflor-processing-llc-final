...
def _delivery_workdir() -> str:
    """Working directory for the delivery runner.

    The repo root is the natural choice, but terminal_tool's workdir allowlist
    rejects paths with characters such as an apostrophe (a checkout under
    ``.../Ashley's Pipeline/...`` blocked every bot-to-bot delivery — Flo
    downstream, 2026-09-08). The runner only needs *a* valid cwd (the command
    references the CLI by absolute path), so fall back to the user's home, then
    the temp dir, when the repo path fails validation.
    """
    candidates = [str(Path(__file__).resolve().parent.parent), str(Path.home()), tempfile.gettempdir()]
    try:
        from tools.terminal_tool import _validate_workdir
    except Exception:  # pragma: no cover - defensive
        return candidates[0]
    for candidate in candidates:
        if _validate_workdir(candidate) is None:
            return candidate
    return candidates[-1]
