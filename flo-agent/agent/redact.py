...
# Hosts whose URLs carry a credential in the query string by design (MCP
# endpoints such as ``https://mcp.zapier.com/api/v1/connect?token=…``).
# Registered by downstream code/plugins; see redact_sensitive_text().
_CREDENTIAL_URL_HOSTS: set = set()
_credential_hosts_lock = threading.Lock()


def register_credential_url_hosts(hosts) -> int:
    """Register hostnames whose URL query strings must be redacted in logs.

    Returns the number of newly registered hosts. Matching is exact,
    case-insensitive, on the URL authority's hostname (userinfo and port
    ignored). Idempotent.
    """
    added = 0
    with _credential_hosts_lock:
        for host in hosts or ():
            if not isinstance(host, str):
                continue
            key = host.strip().lower()
            if key and key not in _CREDENTIAL_URL_HOSTS:
                _CREDENTIAL_URL_HOSTS.add(key)
                added += 1
    return added


def _reset_credential_url_hosts() -> None:
    """Tests only."""
    with _credential_hosts_lock:
        _CREDENTIAL_URL_HOSTS.clear()
...
def _redact_registered_host_url_credentials(text: str) -> str:
    """Mask credential-named query params only for registered hosts."""
    def _sub(m: re.Match) -> str:
        if _authority_host(m.group(2)) not in _CREDENTIAL_URL_HOSTS:
            return m.group(0)
        query = _redact_query_string(m.group(4))
        return f"{m.group(1)}://{m.group(2)}{m.group(3)}?{query}{m.group(5) or ''}"
    return _URL_WITH_QUERY_RE.sub(_sub, text)
...
def redact_sensitive_text(
    ...
    *,
    ...
    elif _CREDENTIAL_URL_HOSTS and "://" in text:
        # Flo downstream (2026-09-08): hosts registered via
        # register_credential_url_hosts() carry their credential in the query
        # string by design (e.g. Zapier MCP ``?token=…``). Those URLs are never
        # round-trip workflow links, so the default log pass masks their
        # credential-named params without changing the pass-through policy for
        # every other host. Kept as an opt-in registry so upstream semantics
        # (OAuth callbacks, magic links) are untouched.
        text = _redact_registered_host_url_credentials(text)
